        ...
        if (dataSet.getColors().size() > 1) {
            ...
            if (e1 != null) {
                ...
                for (int x = mXBounds.min; x <= mXBounds.range + mXBounds.min; x++) {
                    ...
                    //如果值为0则不绘制
                   // if (e1.getY() == 0f || e2.getY() == 0f) continue;
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    //获取下一个值
                    float Offset = -valOffset;
                    if(((j+2) / 2 + mXBounds.min)<dataSet.getEntryCount()){
                        Entry nextEntry = dataSet.getEntryForIndex((j+2) / 2 + mXBounds.min);
                        float nextData = nextEntry.getY();

                        //是否是第一个,j！=0 因为第一个需要在上面
                        //比较当前值和下一个值的差距是否大于一定值
                        if(Math.abs(entry.getY()-nextData)>10&&j!=0){
                            Offset = 2*valOffset;
                        }

                        //比较当前值是不是最大的
                        if(((j-2) / 2 + mXBounds.min)>=0){
                            Entry onEntry = dataSet.getEntryForIndex((j-2) / 2 + mXBounds.min);
                            float onData = onEntry.getY();
                            if(entry.getY()>nextData&&entry.getY()>onData){
                                Offset = -valOffset;
                            }
                        }
                    }
                    ...
                    drawValue(c, dataSet.getValueFormatter(), entry.getY(), entry, i, x,
                            y+Offset, dataSet.getValueTextColor(j / 2));
                    // - valOffset
                }
            }
        }
