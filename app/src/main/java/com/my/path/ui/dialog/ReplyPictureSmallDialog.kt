 ...
 * 评论
 * */
class ReplyPictureSmallDialog(var mActivity: Activity,val hint:String = "", val back: (content:String,path:String)->Unit) : BottomPopupView(mActivity){

    override fun getImplLayoutId():Int = R.layout.public_send_comments2
    ...
    private fun initData(){
        ...
        findViewById<TextView>(R.id.tv_fasong).clickWithDebounce {
            back.invoke(findViewById<EditText>(R.id.tv_comment_neirong).text.toString(),picStr)
            dismiss()
        }
        ...
        findViewById<ImageView>(R.id.imageColse).clickWithDebounce {
            ...
            addTextChanged( findViewById<EditText>(R.id.tv_comment_neirong).text.toString())
        }

        findViewById<View>(R.id.viewsa).clickWithDebounce {
            setImage()
        }
        findViewById<EditText>(R.id.tv_comment_neirong).hint = hint
        //监听输入框
        findViewById<EditText>(R.id.tv_comment_neirong).addTextChangedListener(object : TextWatcher {
        ...
        showInput(findViewById(R.id.tv_comment_neirong))
    }

    private fun addTextChanged(str:String){
        if (str != ""||picStr!="") {
            findViewById<TextView>(R.id.tv_fasong).background = getBackgroundExt(R.drawable.bg_comment_fasong_bg_gai2)
        } else {
            findViewById<TextView>(R.id.tv_fasong).background = getBackgroundExt(R.drawable.bg_comment_fasong_bg_gai)
        }
    }
    ...
    override fun onDismiss() {
        findViewById<EditText>(R.id.tv_comment_neirong)?.apply {
