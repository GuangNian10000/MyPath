...
import com.my.path.databinding.FragmentLoginCodeBinding
 ...
 * @description　: 登录
 */
class LoginCodeFragment : BaseFragment<RegisteredViewModel, FragmentLoginCodeBinding>() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        appViewModel.run {
            userCountryCode.observeInFragment(this@LoginCodeFragment, Observer {
                phoneViewModel.ipCode.set("+$it")
            })
            userCountryName.observeInFragment(this@LoginCodeFragment, Observer {
        ...
        mDatabind.viewTextInputPwd.llPwd.visibility = View.GONE
        ...
        mDatabind.viewTextInputPwd.textView31.text = Html.fromHtml(
            getString(R.string.yiyouzhangshd) + "<font color=\"#FF544C\">" + getString(R.string.denglusadxsa) + "<font/>"
        )
    }
...
//        }
    ...
    private fun check(){
        if(""!=phoneViewModel.region.get()&&
            ...
            requestLoginViewModel.loginCode(
                phoneViewModel.ipCode.get().replace("+",""),
                phoneViewModel.userName.get(),
                codeViewModel.verificationCode.get())
        }else{
